...
extern "C" {
    [[noreturn]] void _stage1Main(uint32_t dx, uint32_t __stop_stage1,
                            uint32_t cursor);
    [[noreturn]] void jumpInit(uint32_t initAddr, uint32_t mbInfo,
                            uint32_t termIndex);
}
...
void _stage1Main( uint32_t dx, uint32_t __stop_stage1, uint32_t cursor)
{
    ...
    out << "Welcome to PatroclusOS 32-bit protected mode bootloader!\n";
    ...
    #ifdef TRACEMAX
        traceOut << "Read file INIT.BIN, size is: " << initFile->size << 
            ", and ptr is " << reinterpret_cast<uint32_t>(initFile->ptr) << "\n";
        traceOut << "Read file KERNEL.BIN, size is: " << kernelFile->size << 
            ", and ptr is " << reinterpret_cast<uint32_t>(kernelFile->ptr) << "\n";
    #endif

    // Find multiboot header
    multiboot_header* mbHeader = multibootHeaderSearch(initFile);
    ...
    elf32_Ehdr *initElfHeader = reinterpret_cast<elf32_Ehdr*>(initFile->ptr);
    if (!(elf_check_file(initElfHeader)))
        earlyPanic("init.bin's ELF header is not recognized, or not supported!");

    void* initEndPtr = elf32_load(initElfHeader);

    if (! initEndPtr )
        earlyPanic("Problems loading ELF binary init.bin");
    ...
    uint32_t initAddr = initElfHeader->e_entry;
    uint32_t mbInfoPtr = reinterpret_cast<uint32_t>(mbInfo);

    jumpInit(initAddr,mbInfoPtr,index);
